...
- **Bekleyen (son checkpoint):** yalnız `payment-tester.tsx` hâlâ `@commerce-os/ui` (paylaşılan) + eski `slate/brand/emerald` palette kullanıyor; DS göçü bekliyor. MOCK-first ödeme guard'ı (`payment-tester.tsx` provider≠MOCK) göç sırasında korunmalı (test bunu bekçiliyor).
