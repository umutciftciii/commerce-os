...
- **Durum.** DONE (worktree; **commit/push/PR/merge/deploy YOK** — brief gereği). Kapsam: apps/storefront-web (lib/search/*, lib/server/search, components/search/*, header-search, product-media, products/page+error+loading), packages/contracts (+PublicSearch tip export), packages/api-client (search yüzeyi), packages/i18n (search bloğu TR/EN). YENİ MIGRATION YOK. Bilinçli kapsam dışı: facet UI (156C), kampanya rozeti (155.2), Load More (156C/D), kategori SEO landing + JSON-LD (156D), autocomplete/suggest/recent (backend yok). Sıradaki: TODO-156C dynamic facet UI.
