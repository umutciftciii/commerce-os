...
## 2026-07-18 — Faz 2C-4: Identity Management Engine (SKU/Barcode/Title pattern motoru) (ADR-073, TODO-150)

- **Kapsam.** 2C-3 persistence altyapısını (kalıcı `ProductVariant`, `combinationKey`, `ProductVariantOptionValue`, deterministik
  placeholder SKU `V-<productId>-<hash>`, `deriveTitle`) tüketip **pattern (kural) tabanlı Identity Management Engine** ekler. Bu faz
  yalnız **SKU · Barcode · Variant Title** aktif (GTIN/EAN/UPC/ERP/marketplace altyapı olarak öngörülür, YAZILMAZ). Combination Engine /
  generation DEĞİŞMEDİ; ayrı kimlik katmanı. **SKU Matrix DEĞİL → Identity Matrix** (pattern editörü + preview + collision + apply).
- **SAF motor** (`apps/api-gateway/src/identity-engine/`): `tokenizer.ts` (lexer; kaçış `{{`/`}}` + charset + dengeli/iç-içe parantez) →
  `parser.ts` (token semantiği + AST; aktif PRODUCT/CATEGORY/ATTRIBUTE:code/COLOR/SIZE/SEQ[:pad], rezerve ID/YEAR/MONTH →
  `IDENTITY_TOKEN_NOT_SUPPORTED`) → `evaluator.ts` (identifier modu=value+UPPER / title modu=label; SEQ padding; eksik token=missing).
  `collision.ts` (internal+external, O(n+m)) + `preview.ts` (değerlendirme+validation+collision; blocking bayrağı). Hepsi Prisma/HTTP/
  `Date`/`Math.random` BİLMEZ. String-replace DEĞİL → her hata stable kod.
- **Veri modeli (additive).** `ProductVariant.titleIsCustom Boolean @default(false)` (title override koruması; varyant PATCH `title`
  verince true) + `enum VariantIdentityField (SKU|BARCODE|TITLE)` + append-only `VariantIdentityChange` (batchId gruplu undo metadata;
  oldValue/newValue/field/pattern/changedByPlatformUserId scalar). Migration `20260718130000_add_identity_management_engine` (1 kolon +
  enum + tablo + 4 index + 3 FK; backfill YOK; down YOK).
- **Servis.** Preview yalnız-okuma + deterministik; apply **server-authoritative** (preview'i YENİDEN hesaplar) + tek
  `prisma.$transaction` + advisory xact lock (`$executeRaw`) + yalnız-değişen yazım + audit. Fail-closed: blocked (SKU collision / sert
  validation) → hiçbir yazım (422 `IDENTITY_APPLY_BLOCKED`). Idempotent (ikinci apply → updated=0). Dış-SKU sahipleri tek `in` sorgusu
  (N+1 YOK). Barcode duplicate = uyarı (DB unique yok).
- **API.** `GET .../identity/preview` (query pattern'lar) + `POST .../identity/apply`. contracts `identity*Schema` + api-client
  `...identity.{preview,apply}` + BFF proxy. Combination/generation uçları BOZULMAZ.
- **UI.** Ürün formuna **Identity Matrix** (`useIdentityMatrix` + `identity-matrix.tsx`): debounce'lu canlı preview, pattern editörü,
  preview tablosu (mevcut→yeni + değişim/çakışma rozetleri), collision paneli, apply (blocked/değişiklik-yok iken pasif); yalnız
  düzenleme + eksen varsa görünür. i18n tr+en (`identityMatrix`).
- **Testler.** api-gateway `identity-engine.test.ts` (~46: tokenizer/parser/evaluator/collision/preview saf + service in-memory fake
  [preview/apply/idempotent/blocked/title-koruma/pattern-hata/no-pattern/product-not-found] + route [GET/POST/422]). Regresyon:
  api-gateway **878/878**, store-admin **285/285**.
- **Gate.** Prisma format+generate + migration SQL + **full `pnpm -r build` (25/25 proje) PASS** + api-gateway/store-admin typecheck
  TEMİZ + tests yeşil.
...
- **Ship + Deploy.** feat `5a5e597` → PR #83 (CI lint·test·build yeşil, 3m37s) → merge `04264ae`=main (repo-standard merge-commit). **YENİ MIGRATION YOK.** Deploy: merged-main'den api-gateway+worker rebuild, migrate deploy up-to-date, **4/4 healthy**, post-merge runtime smoke (health 200 · store-wide · subtree · exact/prefix/trigram · option/color/numeric/boolean/multiselect facet · OR/AND · disjunctive · price/stock · sort/pagination · tenant · 400/404 hata kodları · allowlist sıfır sızıntı) **ALL PASS** + mevcut catalog/PDP regresyon 200. Smoke verisi temizlendi (cascade → read-model 0/0). Sıradaki: TODO-156 (Faz 2C-8C) storefront PLP/filter UI.

## Faz 2C-9 — Search Listing Projection Enrichment (TODO-155.1, ADR-079 Ek)
- **Amaç (R1 çözümü).** Arama kartını, eski public-katalog kartındaki ticari/görsel zenginliği KAYBETMEDEN besleyecek **bounded, tenant-safe, public-safe** listing projection'ı search read-model'e eklemek. Kart, arama yanıtından **ikinci hydration turu OLMADAN** render edilir (read-model-only kilidi korunur; kampanya motoru search'te yeniden çalıştırılmaz; N+1 yok).
- **Veri modeli (ADR-079 Ek).** `ProductSearchDocument` additive: typed skalar `compareAtMinor`/`discountPercent`/`omnibusPreviousPriceMinor` + tek **bounded JSON** `listing` (primaryImage/secondaryImage/swatches[]/swatchTotalCount). Normalize child-tablo REDDEDİLDİ (swatch yalnız görüntü; JSON = sıfır ek sorgu + otomatik stale-cleanup + OpenSearch nested + basit migration). Migration `20260719130000_enrich_search_listing_projection` — **tamamen additive** (4 nullable ADD COLUMN; DEFAULT yok → metadata-only; inline veri taşıma YOK; runtime backfill/reindex ile dolar).
- **Builder (SAF).** En ucuz görünür varyanttan ticari snapshot (tek indirim formülü; Omnibus yalnız indirim aktifken + gerçek `ProductPriceChange` verisi varsa, sahte üretilmez). Swatch = ACTIVE option ∩ ACTIVE varyantı olan (dedupe · sortOrder · archived/inactive hariç · option görseli yoksa ana görsele fallback · deterministik default · bounded 8 + `swatchTotalCount`). Görsel = primary (position ASC) + secondary (farklı mediaId, hover); IÇ `storageKey`+boyut. `now`-pencere IO katmanında → builder SAF kalır.
- **Data katmanı.** `loadSources` bounded batch'e 4 sorgu ekler (omnibus groupBy · ProductImage+MediaAsset · media-axis AttributeOption · ProductVariantOptionValue) — chunk başına SABİT, N+1 YOK, storeId scope'lu (PLATFORM option `OR storeId null`). `applyBuild` yeni kolonları yazar (Json null → `Prisma.DbNull`).
- **Route + DTO.** Kart görselleri artık read-model `listing` snapshot'ından; url **route'ta** `resolveMediaUrl` ile türetilir (eski `resolveCovers` ProductImage sorgusu KALDIRILDI). `publicSearchProductSchema` additive (`compareAtMinor`/`discountPercent`/`omnibusPreviousPriceMinor`/`secondaryImage`/`swatches[]`/`swatchTotalCount`; default'lu → eski istemci kırılmaz). `storageKey`/`mediaId` allowlist + zod strip ile **DTO'ya sızmaz**.
- **Reindex.** Bu pass için YENİ tetikleyici GEREKMEZ (mevcut reindexProduct/reindexStore swatch/commercial/omnibus tazeliğini kapsar). PLATFORM option = TD-049 duruşu (admin rebuild). Kampanya rozeti snapshot'ı **TODO-155.2'ye ERTELENDİ** (read-time pencere bastırma + lifecycle event stratejisi ADR'de sabit); Omnibus pencere kayması → günlük sweep (dokümante).
- **Testler + gate.** search-service **70** (+21 listing-projection: promosuz/indirimli/compareAt/discount%/Omnibus var-yok/fiyat-gizli/primary-secondary/swatch yok-tek-çoklu/dedupe/sortOrder/archived/inactive/fallback/default/bounded) + contracts **110** (+3 DTO allowlist/additive) + api-gateway **1047** (search-public route yeni alan + allowlist assertion güncellendi). db:generate → **full build 24/24** (worker/storefront/store-admin dahil) → typecheck → lint TEMİZ.
- **Docker gerçek-PG runtime smoke (ALL PASS).** İzole worktree'den api-gateway+worker rebuild (in-image build, 4/4 healthy) → migrate deploy `20260719130000` uygulandı → demo-store backfill (scanned 8, **indexed 3**, removed 5). **Kanıtlar:** (1) jsonb `listing` round-trip gerçek PG'de; (2) Demo Hoodie **3 swatch** (Siyah/Kırmızı/Mavi + colorHex + **per-option görsel binding** + `isDefault`=en-ucuz-varyant rengi Kırmızı) — gerçek Renk ekseninden; (3) primary+secondary görsel url `storageKey`'den route'ta türetildi; (4) görselsiz/eksen-siz ürün → `listing` null (placeholder fallback); (5) **ticari yansıma**: variant compareAt=price·5/4 set → reindex → `compareAtMinor`=187375 + `discountPercent`=**20** hem DB hem endpoint'te; (6) **backfill idempotency**: 2. koşu revision 0→1 (upsert update, dup yok); (7) **allowlist SIFIR sızıntı** (storageKey/mediaId/checksum/costMinor/netPriceMinor/iç `listing` adı DTO'da YOK). Smoke verisi **geri alındı** (compareAt→NULL + reindex; compareAt/discount temiz). Not: option-archive/media-axis-change/Omnibus canlı senaryoları 21 birim testinde kapsanır (tek-store ortam; cross-store storeId-scope ile garanti).
- **Ship + Deploy.** feat `dbeeac0` → PR **#85** (CI lint·test·build **pass 3m34s**) → merge **`42bc9c7`**=main=origin/main (repo-standard merge-commit). Migration `20260719130000` (additive). Deploy: **merged-main'den** api-gateway+worker rebuild (main repo context), migrate deploy **up-to-date**, **4/4 healthy**, post-merge runtime smoke **ALL PASS**: health 200 · backfill (indexed 3, idempotency 2. koşu OK) · Demo Hoodie **3 swatch** (per-option görsel + isDefault=Kırmızı) · primary+secondary(hover) url · **compareAt→discountPercent %20** (DB+endpoint) · Omnibus null-path · **ProductCard tek endpoint'ten render** · **allowlist sıfır sızıntı** · facet/search regresyon yok (facet pipeline çalışır; bu dataset'te filterable attribute yok → 0 facet, kod değişmedi) · tenant storeId-scope. Smoke verisi **geri alındı** (compareAt→NULL + reindex). Sıradaki: TODO-155.2 (kampanya rozeti snapshot + reconciliation sweep) veya TODO-156B (storefront wiring).

## Faz 2C-8C — Storefront Search Experience Foundation (TODO-156B, ANALIZ-156A)
- **Amaç.** PLP'yi (`/products`) eski "hepsini çek + istemcide filtrele" akışından **URL-state + RSC + public search endpoint** temeline geçirmek. Kapsam: URL-state codec + SSR PLP wiring + listing adapter + kart + sort + numaralı pagination + header search + loading/empty/error UX + SEO temeli + a11y. **Facet UI (156C) ve kampanya rozeti (155.2) bilinçle kapsam dışı.**
- **Route kararı.** ANALIZ §8.2 kilidi korundu: **ayrı `/search` yok**; `/products` tek arama yüzeyi (`q` + `category` + `sort` + `page` + fiyat/stok + `filter[...]` passthrough). Header formu `action="/products"` zaten uyumlu. Kategori landing (`/categories/[slug]`) + JSON-LD 156D'de.
- **URL-state codec (TEK OTORITE).** `lib/search/url-state.ts` (SAF, Next importsuz): parse (lenient, bozuk deep-link sessizce varsayılana iner) + kanonik serialize (deterministik anahtar sırası, alfabetik filtre değerleri, VARSAYILANLAR atılır → aynı seçim = aynı URL) + mutasyonlar (filtre/sort/q/kategori/fiyat/stok değişince **page=1**). Bilinmeyen query paramı **korunmaz** (kontrollü eleme; canonical temizliği + SEO). Gateway parser'ıyla birebir (`apps/api-gateway/src/search/query-parser.ts`). Serialize çıktısı **hem tarayıcı URL'i hem gateway sorgusudur** (tek serializer).
- **RSC/client sınırı.** RSC: `page.tsx` (searchParams → BFF fetch → SSR), `SearchHeading`/`SearchToolbar`/`ProductGrid`/`SearchEmpty` (sunucu). Client island'lar: `SearchTransitionProvider` (useRouter+useTransition), `SortControl`, `SearchPagination`, `SearchProductCard` (swatch/hover), `HeaderSearch` (useSearchParams prefill). İstemci **hiçbir arama mantığı çalıştırmaz**; yalnız URL günceller. İlk yüklemede ek fetch YOK; ürün başına hidrasyon YOK; `/products` route client bundle **4.26 kB** (arama iş mantığı client'a taşınmadı — §18).
- **BFF (sunucu-yalnız).** `lib/server/search.ts` → gateway `GET /public/stores/:slug/search` (`no-store`) → yanıt **`publicSearchResponseSchema` ALLOWLIST parse** (schema failure → kontrollü error; internal alan sızmaz). Hata eşleme: 404→no-store/category-not-found, 400→bad-request (kurtarılabilir boş durum), 5xx/ağ/parse→error (route error boundary). Tek kanal: api-client `PublicSearchResponse` tipi + `publicSearchResponseSchema` değer re-export (client bundle'a sızmaz; contracts'a doğrudan bağlanılmaz).
- **Listing adapter.** `lib/search/listing-adapter.ts` (SAF): search product DTO → `SearchListingCard`. YALNIZCA biçimleme (minor→tr-TR); **ticari hesap tekrarı YOK** (indirim/Omnibus/compareAt sunucuda). title/href/category/price/compareAt/discountPercent/Omnibus/primary+secondary image/swatches/swatchTotalCount/defaultSwatch/currency. Eksik alanlarda kontrollü fallback (kampanya yok → kart kırılmaz; görsel yok → placeholder). Internal read-model alanı (storageKey/mediaId/searchVector/cost) taşınmaz.
- **ProductCard (search).** `components/search/search-product-card.tsx` — StorefrontProductSummary kartından AYRI (search projection'a özel). Görsel öncelik (§8): aktif swatch preview → secondary hover (`[@media(hover:hover)]:group-hover`, touch varsayımı yok, reduced-motion) → primary. Swatch = YALNIZ görsel önizleme (hover/focus preview, ayrılınca default; varyant seçimi/SKU/fiyat/sepet kararı YOK); colorHex yoksa erişilebilir label baş harfi; `+N`. İndirim rozeti nötr `ink` (aksan yok). Stok dışı metin rozeti. Mevcut `ui/product-card.tsx` (PDP related/home) KORUNDU.
- **Toolbar + Sort + Pagination.** Toolbar: sonuç sayısı `aria-live="polite"` + SortControl (sticky). Sort: yalnız backend'in 6 anahtarı (relevance/newest/price_asc/price_desc/title_asc/title_desc; SAHTE seçenek yok); değişince URL replace + page=1 + pending. Pagination: SSR numaralı (deterministik href codec'ten; `paginationRange` saf pencere), gerçek `<Link>` (JS'siz çalışır) + `useTransition` push (geri/ileri sayfalar arası adımlar, sayfa değişince en üste scroll), `aria-current="page"`, prev/next disabled. **Load More: 156C/156D'ye ERTELENDİ** — client'ın gateway'e doğrudan erişimi yok (BFF sunucu-yalnız); temiz+history-güvenli Load More ayrı route-handler/server-action gerektirir → §11 "yarım/history-bozan çözüm yapma" gereği ertelendi; numaralı pagination canonical otorite.
- **Loading/Empty/Error.** loading.tsx (`aria-busy` + sr-only status + gerçek kart ölçüsünde skeleton; tam ekran spinner yok). Empty: 4 ayrı durum (mağaza boş / arama sonuç yok / kategori boş / filtre sonuç yok) + aktif parametre özeti + "aramayı/filtreleri temizle" + "tüm ürünler" (yanıltıcı öneri yok). error.tsx (route boundary; internal mesaj gizli; "Tekrar dene" reset; katalog/PDP erişilebilir kalır).
- **Header search.** `components/site/header-search.tsx` — MOCK kaldırıldı. Gerçek GET form (`action="/products"`, JS'siz de çalışır) + `useSearchParams` prefill (Suspense + static fallback). Submit → gerçek SSR PLP. Bu fazda YOK: autocomplete/suggest/recent/debounce.
- **SEO temeli (§16).** `lib/search/seo.ts` (SAF): düz PLP + kategori-yalnız `index,follow`; arama (q) + fiyat/stok/dinamik filtre `noindex,follow` (arama sonuç + filtre kombinasyon patlaması indexlenmez). Canonical: indexable → kategori+page normalize (sort/filtre düşer); noindex → self (ana kategoriye **ezme YOK**). Metadata title (arama/sayfa farkında) + breadcrumb iskeleti (görünür; JSON-LD 156D).
- **Eski akış temizliği.** `components/site/product-listing.tsx` (istemci filtre/slice `ProductListingView`) + eski `product-listing-page.test.tsx` **silindi** (yalnız `/products` kullanıyordu → ölü kod). `getStorefrontListing`/catalog + eski `.../products` gateway ucu **KORUNDU** (home featured + PDP related hâlâ kullanır; kırılmadı).
- **A11y (WCAG 2.1 AA).** semantic main/nav/form (role="search"), sort label, pagination `nav[aria-label]`+`aria-current`, swatch button label (renk tek başına anlam taşımaz), görünür focus ring (`--accent`), `aria-live` sonuç sayısı + `aria-busy` pending, empty/error heading, reduced-motion. Responsive: 2 (mobil) / 3 (tablet) / 4 (desktop) kolon; sabit 4:5 oran (layout shift min).
- **Testler + gate.** storefront **+75 yeni test** (url-state 24 · listing-adapter 11 · pagination 7 · seo 8 · products-search-page 11 SSR · search-product-card 10 · header-search 4) → **273/273 PASS**; i18n parity 47 (TR/EN `search` bloğu birebir) + contracts 110. Değişen paketler build (utils/contracts/i18n/api-client/ui) → **`next build` yeşil** (Compiled + type-check + 4/4 static) → **lint temiz**. NOT: `apps/storefront-web` typecheck GATE'i yok; manuel `tsc` pre-existing bir test-fixture tip borcunu (`checkout-form-render.test.tsx` stale CartLineView) gösterir — kapsam dışı, `next build`'i kırmaz, vitest runtime geçer (background task ile flag'lendi).
- **Docker runtime smoke (worktree kodu; ALL PASS).** Çalışan merged-main stack'e (postgres/redis/api-gateway :4000 healthy) karşı worktree production build `next start :3001`. Kanıtlar: health 200 · **PLP SSR** (3 ürün: P1A Single ₺180 / Demo Tote ₺399 / Demo Hoodie ₺1.499 + breadcrumb + "3 ÜRÜN" aria-live + 6 sort seçeneği) · **header arama** submit → `/products?q=hoodie` (native GET) → Demo Hoodie 1 sonuç · q prefill "hoodie" · **exact** q=single → P1A Single · **sort** SSR seçili değer · **pagination** pageSize=2 → [1]/2, sayfa 2 click → `?pageSize=2&page=2` push + Demo Hoodie + başlık "Sayfa 2" + Sonraki disabled · **geri/ileri** history.back → sayfa 1 (P1A+Tote) · **primary+secondary** görsel (hoodie) · **swatch** 3 buton (Renk: Siyah/Kırmızı/Mavi erişilebilir label) · **sticky toolbar** · **mobil** 2 kolon (375px) · **regresyon** PDP `/products/demo-hoodie` 200 (Sepete ekle/Ürün açıklaması) + home 200 + eski katalog ucu 200. Konsol hatası yok. Smoke verisi üretilmedi (mevcut read-model kullanıldı → temizlik gerekmez).
- **Ship + Deploy.** feat `415a0cd` → PR **#87** (CI lint·test·build **pass 3m37s**) → merge **`77042e4`**=main=origin/main (repo-standard merge-commit). **YENİ MIGRATION YOK** (`prisma migrate deploy` → "No pending migrations"). Deploy: **merged-main'den** storefront-web rebuild (in-image package build; host build gerekmez — TD-137), `docker compose up -d`, **5/5 healthy** (postgres·redis·api-gateway·worker·storefront-web). **Post-merge runtime smoke (merged-main container :3000) ALL PASS:** storefront+gateway health 200 · /products SSR (3 ürün) · header arama q=hoodie · exact/no-result(q=zzzzz) · sort SSR-selected · pagination click sayfa 2 (push) + geri/ileri sayfa 1 · swatch 3 buton · mobil 2-kolon (375px) · **browser'dan search fetch YOK** (tek sunucu-yanı çağrı; ilk render'da client fetch yok) · konsol temiz · regresyon PDP/home/cart 200 + eski katalog ucu 200. Smoke verisi üretilmedi (mevcut read-model). Kapsam: apps/storefront-web + packages/{contracts,api-client,i18n}. Bilinçli kapsam dışı: facet UI (156C), kampanya rozeti (155.2), Load More (156C/D), kategori SEO landing + JSON-LD (156D), autocomplete/suggest/recent (156E). Sıradaki: **TODO-156C** dynamic facet experience.

## Faz 2C-8D — Dynamic Facet Experience (TODO-156C, ANALIZ-156A §5-§7)
- **Amaç.** 156B'nin URL-state + RSC + public search temeline **veri-güdümlü facet deneyimi** eklemek: desktop kalıcı filtre rayı + mobil tam-yükseklik drawer (aynı renderer) + aktif filtre çipleri + `FacetRenderer` registry (SELECT/MULTI_SELECT/COLOR/BOOLEAN/INTEGER/DECIMAL/DATE). Backend disjunctive facet zaten dönüyordu (TODO-155); bu faz YALNIZ render + çip + drawer + URL mutasyonu ekler. **Backend/şema/migration DEĞİŞMEDİ.**
- **Facet mimarisi (registry, switch-case YOK).** `lib/search/facets.ts` `resolveFacetKind(facet)` iki katmanlı anahtarı (BİRİNCİL `selectionMode`, İKİNCİL `dataType`) tek render-türü stringine indirir: RANGE+DATE→`date`, RANGE→`range`, BOOLEAN→`boolean`, MULTI+COLOR→`color`, MULTI→`checkbox`, bilinmeyen→`checkbox` (fallback, asla patlamaz). `components/search/facets/registry.tsx` `Record<FacetKind, Component>` lookup. **Yeni attribute tipi = resolver'a bir dal + registry'ye bir satır; PLP/rail/drawer değişmez** (backend "hardcode facet yok" ilkesinin frontend karşılığı).
- **Tek renderer (rail = drawer).** `facets/facet-list.tsx` (`memo`) paylaşılan render: Fiyat → Stok → dinamik facet'ler (backend `displayOrder`). Her facet `FacetSection` (collapse/expand accordion, aktif seçim rozeti) + registry `FacetControl`. Desktop `FilterRail` (≥lg, sticky+scroll, `hidden lg:block`) ve mobil `FilterDrawer` (<lg) İKİSİ DE bu component'i sarar — kopya YOK.
- **Facet tip bileşenleri.** `FacetValueList` (checkbox; SELECT/MULTI_SELECT/TEXT **ve BOOLEAN** paylaşır — brief §7 "switch yerine checkbox"; count + disabled(count=0&seçili değil) + "daha fazla göster" ilk 8) · `FacetColorSwatch` (gerçek `colorHex` daire + seçili halka/onay + tooltip + label + count + fallback nötr swatch; ilk 12) · `FacetNumberRange` (min/max sayısal input, `unit` eki, SLIDER/HISTOGRAM YOK — §6.3; input taslağı `key` ile URL'e reseed) · `FacetDateRange` (native `type=date` ↔ epoch-ms UTC; "hazırlık", backend DATE facet gönderirse çalışır) · `PriceFacet` (top-level minPrice/maxPrice, ana birim ₺ ↔ minor ×100) · `StockFacet` (top-level inStock, tek checkbox).
- **Aktif filtre çipleri.** `active-filter-chips.tsx` + `deriveActiveChips(state, facets, {t, currency})` çipleri **YALNIZCA URL state'ten** türetir (facet meta yalnız ETİKET; stale değer→ham value). Sıra: arama→kategori→fiyat→stok→dinamik. Her çip tekil "×" (`removeHref`=buildSearchHref+saf mutasyon, kanonik, diğer paramları korur) + "Tümünü temizle" (`clearedSearchState`). Gerçek `<Link>` (JS'siz çalışır) + `useTransition` replace.
- **URL mutasyonları (SAF, tek yazma noktası).** `url-state.ts`'e eklendi: `toggleFilterValue`/`removeFilterValue`/`removeFilter`/`setFilterRange`/`withPrice`/`withInStock`/`clearedFiltersOnly`. Hepsi kopya döner (girdiyi mutate etmez), page→1, bosalan filtre kodunu siler. **Yerel filtre state YOK**: her etkileşim URL'i günceller (replace), RSC sunucuda yeniden fetch eder, grid SSR ile tazelenir. Mobil drawer "anında uygula" (canlı sayaç "n Ürünü Göster"; open-state salt UI); "gecikmeli uygula"nın yerel-kopya riski bilinçle alınmadı (brief §10/§11 "URL tek otorite").
- **A11y (WCAG 2.1 AA).** Drawer: `role=dialog`+`aria-modal`, ESC kapatır, body scroll-lock, **focus-trap** (Tab döngüsü, ilk odak kapat butonu, kapanınca odak tetikleyiciye döner). Facet section `<button aria-expanded/aria-controls>` (içerik `hidden` ile DOM'da kalır). Checkbox/swatch native `<input>` (klavye+aria doğal); swatch görsel gizli input + `aria-label` (renk tek başına anlam taşımaz). Çip `aria-label="{group}: {value} filtresini kaldır"`. Görünür focus ring (`--accent`). Responsive: desktop rail → tablet/mobil drawer (tek renderer); grid 2/3/4 kolon korundu.
- **Testler + gate.** storefront **+39 yeni test** (`search-facets` 29: url mutasyonları + `resolveFacetKind` tam matris + sayaçlar + çip türetimi/sıra/stale/removeHref · `search-facets-render` 10 SSR: her registry tipi + FacetList + chips + rail + drawer trigger) → **312/312 PASS** (37 dosya). i18n parity **47** (TR/EN `search` bloğu birebir; +25 anahtar). **`next build` yeşil** (type-check + 4/4 static; `/products` 9.63 kB) · **lint temiz**. Değişen: `apps/storefront-web` + `packages/i18n`. **YENİ MIGRATION YOK.**
- **Docker runtime smoke (worktree imajı; ALL PASS).** worktree kodundan `storefront-web` rebuild → `up -d` (7/7 healthy). Facet verisi geçici seed'lendi (COLOR renk + INTEGER agirlik + BOOLEAN organik, `seed156c-*`; `filterable` CategoryAttribute + ProductFacetValue). Kanıtlar: **SSR** rail+facet markerları (Filtreler/Fiyat/Stok/Renk/Ağırlık/Organik/Siyah/`background-color:#111111`/En az) · **desktop** Siyah swatch tık → `?filter[renk]=siyah` + "2 ürün" (disjunctive: Kırmızı/Tote düştü) + çip "Renk: Siyah" + checkbox selected + agirlik range disjunctive daralma (200→500 min) · **çip kaldırma** → `?sort=price_asc` (renk düşer, sort korunur) → 3 ürün · **sort+facet kompozisyonu** · **mobil drawer** aç → `role=dialog`+`aria-modal=true`+scroll-lock+focus kapat-butonunda+FacetList içeride+footer "3 ÜRÜNÜ GÖSTER"/"TEMİZLE" · **ESC** → kapandı+scroll-lock release+odak tetikleyiciye döndü · **deep-link/refresh/share** 3 kombine filtre (renk=siyah&organik=true&inStock=1) → 3 çip + "2 ürün" + tüm checkbox selected + URL birebir · **geri** → /products 3 ürün · **konsol temiz**. Endpoint disjunctive doğrudan doğrulandı (renk=siyah→2, agirlik min=600→1 P1A, organik=true→2). **Seed verisi geri alındı** (9 PFV+2 CA+2 def silindi → facets 0/3 ürün; mağaza eski hâlinde).
- **Bilinçli kapsam dışı.** Slider (→ayrı faz; §6.3 min/max input) · Load More (→156C/D; BFF sunucu-yalnız) · kategori SEO landing + JSON-LD (→156D) · autocomplete/recent/trending (→156E) · kampanya rozeti (→155.2). Bkz. TD-051.1 (RESOLVED) + TD-053.
- **Durum.** Implementation + gate + worktree docker smoke TAMAM. **Commit/PR/merge/deploy YAPILMADI** (brief "commit öncesi hazır mı" + "tamamlanınca dur"): çalışma ağacı commit'e hazır; ship kararı kullanıcıya bırakıldı. Sıradaki: **TODO-156D** (kategori SEO + a11y sertleştirme).

## Faz 2C-9B — Search Listing Semantic Completion (TODO-155.2, ADR-079 Ek)
- **Amaç.** Kullanıcının gösterdiği İKİ gerçek tutarsızlığı kapatmak: (A) PDP "Sepette %10 / ₺1.349,10" gösterir ama PLP/search kartı yalnız ₺1.499 (kampanya read-model'de yok); (B) Demo Hoodie kartında renk swatch'ları (Siyah/Kırmızı/Mavi) var ama filter rail'de Renk facet YOK. İkisi de regresyon değil, **read-model boşluğu**; bu faz tek fazda kapatır. **checkout nihai fiyat otoritesidir; PLP kampanya fiyatı bilgilendirici tahmindir (ADR-062).**
- **(A1) Paylaşılan kampanya değerlendiricisi.** `selectPublicCampaignDisplay` + `isBadgeEligible`/`campaignAppliesToProduct`/`computeAutomaticEstimate`/`selectPublicCampaignSlides`/`toCouponDisplayFields` + `CampaignRecord`/`CampaignCouponRecord` tipleri api-gateway'den **`@commerce-os/contracts`'a TAŞINDI** (F4A "tek formül" — ADR-062). api-gateway `campaigns/data.ts` + `campaigns/public-badge.ts` artık RE-EXPORT (mevcut içe aktarımlar kırılmadı; PDP yolu DEĞİŞMEDİ — `campaigns-public-badge.test.ts` + 1057 gateway testi yeşil). Yeni: `selectIndexableCampaignSnapshot` (birincil rozet + kazanan `startsAt`/effektif `endsAt`) + `isCampaignSnapshotDisplayable` (read-time bastırma). SAF, `now` parametre → deterministik.
- **(A2) Listing campaign projection.** `ProductSearchDocument`: additive `campaign` (jsonb = PublicCampaignBadge, allowlist), `campaignStartsAt`, `campaignEndsAt` (+`[storeId, campaignEndsAt]` index) — migration `20260719140000` (ADD COLUMN nullable, metadata-only, inline backfill YOK). `document-builder` en ucuz görünür varyant fiyatı (`minPriceMinor`) + store kampanyaları + `evaluationNow` (data katmanı; builder SAF) → `selectIndexableCampaignSnapshot`. Data loader store kampanyalarını BİR KEZ yükler (batch başına tek sorgu; N+1 yok) + ürün `categoryIds` (ProductCategoryAssignment). `campaign` allowlist: iç id/limit/priority/stackable/usageCount SIZMAZ (canlı doğrulandı).
- **(A3) Read-time bastırma.** `search-query.ts::toResultItem(now)` snapshot penceresi `now`'a göre geçersizse (henüz başlamamış / süresi geçmiş) `campaign`'i null'lar → bayat badge vitrine SIZMAZ. Provider-bağımsız (Postgres bugün + gelecekte OpenSearch AYNI semantik). Public DTO (`publicSearchProductSchema.campaign`) additive + gateway search route projeksiyonu.
- **(A4) Lifecycle reindex + reconciliation.** Kampanya create/update/activate/pause/archive → `onCampaignChanged(storeId)` → `searchIndex.reindexStore` (attribute ŞEMA değişimi deseniyle; her zaman doğru + bounded). Zaman-sınırı geçişleri (event'siz) için `CampaignReconcileService` (SAF çekirdek + Prisma persistence) + `reconcile-worker` (TODO-129 sync-worker deseni: setTimeout zinciri + overlap lock + `*_ENABLED` gate, default KAPALI) — (a) süresi geçmiş snapshot'lı ürünleri reindexProduct (b) yeni açılan kampanya mağazalarını reindexStore. main.ts'e bağlandı + shutdown'da durur. Enqueue-only (ağır indeksleme worker'da; idempotent).
- **(B) Variant-defining facet projection (kök boşluk).** `buildFacets` variantDefining dalı artık `VariantAttributeValue` (Faz 2A) **VE** `ProductVariantOptionValue` (ADR-072 eksen seçimleri) değerlerini BİRLEŞİK dedupe (`seen`) ile yayar; ARCHIVED option hariç; yalnız ACTIVE varyant (loader zaten ACTIVE yükler). Loader tüm variantDefining+filterable eksenler için PVOV+option meta yükler (media ekseni sorgusundan ayrı, bounded). Swatch'ı besleyen aynı eksen artık facet'e de yansır → **manuel seed GEREKMEZ**. `filterable=false`/ARCHIVED elenir; disjunctive count/selected semantiği korunur.
- **(C) Storefront kart.** `listing-adapter.toCardCampaign` (PublicCampaignBadge → kart "Sepette" görünümü; istemci HESAP YAPMAZ — nihai fiyat sunucunun güvenli tahmini) + `SearchProductCard` PriceBlock. ÖNCELİK (belgeli): otomatik kampanya → "Sepette" bloğu (final bold + liste üstü-çizili + %rozet + "inCart"); compareAt markdown + Omnibus bu dalda GİZLİ (kampanya güncel teklif). Kampanya yoksa compareAt+Omnibus (F4C). İkincil kupon karta yansımaz (PDP canlı gösterir; TD-054.5).
- **Testler + gate.** search-service **+8** (document-builder: variant-eksen facet 4 + kampanya snapshot 4 → 78) · api-gateway **+10** (reconcile 4 + snapshot/suppression 6 → 1057) · storefront **+5** (adapter 3 + kart 2 → 317). **full build 25/25 · i18n parity 47 · lint · next build · git diff --check TEMİZ.** Migration `20260719140000` additive.
- **Docker runtime smoke (worktree imajları; ALL PASS).** api-gateway+worker+storefront rebuild → migrate deploy (`20260719140000`) → backfill (demo-store, 3 indexed) → 5/5 healthy. **GERÇEK Demo Hoodie (seed YOK):** endpoint kampanya AUTOMATIC_CART_DISCOUNT %10 estFinal 134910 endsAt 2026-07-31 = PDP endpoint ile birebir · **PLP kartı "₺1.349,10 · ₺1.499,00(üstü-çizili) · Sepette · %10"** (ekran görüntüsü) · **Renk facet auto** Siyah/Kırmızı/Mavi doğru colorHex+optionId (PVOV'dan; seed yok) · disjunctive filter[renk]=siyah→1 ürün + sayaçlar korunur · **canlı read-time suppression** (endsAt→2020 → campaign=null; backfill ile geri alındı) · allowlist sıfır sızıntı ("priority" yalnız LCP img prop'u) · **PDP regresyon** SSR+endpoint "Sepette/1.349,10/1.499,00/Kod gerekmez" korundu · reconcile worker "disabled" (default) · konsol temiz · regresyon home/cart/pdp/gateway 200. **Geçici smoke verisi üretilmedi** (gerçek veri); tek geçici mutasyon (endsAt) geri alındı.
- **Durum.** Implementation + gate + docker smoke TAMAM. **Commit/PR/merge/deploy YAPILMADI** (brief kuralı: TODO-156C ile birleşik final review + ship ayrı promptla). Çalışma ağacı commit'e hazır; TODO-156C değişiklikleri de commit edilmedi (birlikte bekliyor). Sıradaki: **birleşik final review + shipping.**

## TODO-156D — SEO URL Governance, Category SEO, JSON-LD & Accessibility Hardening (Faz 2C-8D) — worktree, ship bekliyor

**Kapsam.** Search v1 SEO katmanı enterprise seviyesine. SEO YALNIZ storefront'ta çözüldü; search read-model/`SearchProvider`/Postgres kontratı DEĞİŞMEDİ (ADR-079 korundu). ADR-080 (URL governance + canonical/robots/indexing/sitemap + 404 + a11y), ADR-081 (slug motoru + SlugHistory), ADR-082 (redirect motoru + Redirect), ADR-083 (JSON-LD + public seoTitle/seoDescription).

- **(1) Pure domain (çerçeve-bağımsız, `@commerce-os/utils`).** `slug.ts` (Türkçe transliterasyon + NFKD + reserved + deterministik kolizyon + fallback + validate/generate) · `redirect.ts` (normalize + index + chain-collapse + loop-guard + precedence; 301/302/307/308). Storefront + gelecekteki Admin servisi PAYLAŞIR (brief §19).
- **(2) Storefront SEO katmanı (`lib/seo/`).** `site-url.ts` (mutlak URL otoritesi + metadataBase) · `routes.ts` (URL governance builder'ları) · `breadcrumb.ts` (UI+JSON-LD tek kaynak) · `metadata.ts` (merkezî `buildMetadata`) · `json-ld.ts` (Organization/WebSite+SearchAction/BreadcrumbList/ItemList/Product+Offer/AggregateOffer/Brand) · `product-seo.ts` (title/desc/offer türetme). Bileşenler: `components/seo/json-ld.tsx` (`<` kaçışlı script) + `components/seo/breadcrumb.tsx` (erişilebilir).
- **(3) Route wiring.** `layout.tsx` (metadataBase + title template + Organization/WebSite JSON-LD) · `page.tsx` home metadata · `products/page.tsx` (merkezî metadata + ItemList JSON-LD, indexlenebilir sayfa) · `products/[handle]/page.tsx` (**generateMetadata YOK'tan VAR** — canonical/OG/Twitter/robots + Product+BreadcrumbList JSON-LD + `notFound()` 404 + paylaşılan breadcrumb + React `cache()` çift-fetch önler) · `search-heading.tsx` (kategori-farkında H1 + paylaşılan breadcrumb + BreadcrumbList JSON-LD) · **YENİ** `app/robots.ts` · `app/sitemap.ts` · `app/not-found.tsx`.
- **(4) Public DTO (additive).** `publicProductDetailSchema` + `seoTitle`/`seoDescription` (nullable, public SEO meta metni; internal DEĞİL). Gateway detay handler + api-client tip + storefront `toDetail`/`StorefrontProductDetail`. Search read-model dokunulmadı.
- **(5) Persistence temeli.** `SlugHistory` + `Redirect` (+ `SlugEntityType`/`RedirectType` enum), migration `20260719150000` (additive; yeni tablo, mevcut tabloya kolon YOK; nullable-safe). Store back-relation'ları eklendi. **Henüz gateway/UI'ya bağlı DEĞİL** (TD-057/058/059).
- **(6) Accessibility.** 156B/156C temeli WCAG AA (focus-trap/ESC/scroll-lock, aria-current pagination, aria-busy/aria-live, focus-visible, motion-reduce, skip-link). Bu faz: breadcrumb semantik düzeltmesi (paylaşılan `<Breadcrumb>`; yanlış aria-label + ham slug gösterimi giderildi).
- **Testler + gate.** `@commerce-os/utils` **+50** (slug 30 + redirect 20) · storefront **+31** (seo builder 29 + PDP JSON-LD render 2; PDP not-found artık 404 assert) · api-gateway health test seoTitle/seoDescription public-alan doğrulaması güncellendi. **Tam unit suite 2068/2068 PASS** (166 dosya). **`next build` 17 route temiz** (robots.txt static ○, sitemap.xml/PLP/PDP dynamic ƒ). Storefront tsc temiz (yalnız ÖNCEDEN VAR TD-056). api-gateway tsc temiz. prisma validate + generate OK.
- **Durum.** Implementation + gate + `next build` doğrulaması TAMAM. **Commit/PR/merge/deploy YAPILMADI** (brief kuralı; kod worktree'de hazır). Docker/Lighthouse SEO smoke = deploy adımı (TD-063). Sıradaki: ayrı shipping promptu.

## TODO-156D Tamamlama — SlugHistory Write Path & Runtime Redirect Resolution — worktree, ship bekliyor

**Kapsam.** TODO-156D'de foundation bırakılan iki production eksiği kapatıldı: (1) slug değişince SlugHistory + otomatik 301 redirect YAZIMI, (2) storefront RUNTIME redirect çözümleme. Search read-model / Search API / metadata / JSON-LD / canonical DEĞİŞMEDİ. Migration YOK (modeller önceki fazda additive eklenmişti). TD-058 + TD-059 KAPANDI.

- **(1) Write path (gateway).** YENİ `apps/api-gateway/src/seo/slug-governance.ts::recordSlugChange(tx, params)` — çağıran transaction İÇİNDE atomik: SlugHistory upsert (immutable create-or-noop → duplicate yok) + chain collapse (hedefi eski path olanları yeni path'e repoint) + source=newPath silme (self/back-rename/loop tohumu temizliği) + eski→yeni 301 redirect upsert. `updateProduct` (mevcut `$transaction`'a slug-change dalı + `actorId`) ve `updateCategory` (artık `$transaction`) slug GERÇEKTEN değişince (yeni slug + eski≠yeni) çağırır. Path şekli `@commerce-os/utils` (productUrlPath/categoryUrlPath) TEK KAYNAK. `createdBy` = platform user id.
- **(2) Runtime resolution (storefront).** YENİ `middleware.ts` (404'ten ÖNCE, matcher statik/altyapı hariç) → `lib/seo/redirect-runtime.ts::resolveIncomingRedirect` → public redirect ucundan (TTL 60s modül cache) çekilen kuralları SAF `resolveRedirect` (chain collapse + loop guard) ile çözer → doğru 301/302/307/308 + mutlak Location; yoksa `NextResponse.next()`. Broken/loop/missing → null → geçiş (soft redirect YOK, ana sayfaya redirect YOK). Query-tabanlı (kategori) kaynaklar runtime index'ten HARİÇ (yanlış `/products` redirect'i önlenir; TD-064).
- **(3) Gateway public uç.** YENİ `GET /public/stores/:slug/redirects` — yalnız enabled kurallar; `redirectEnumToStatus` (utils, DB enum→sayısal) allowlist projeksiyon (source/target/status; iç alan sızmaz). `AppDataAccess.listEnabledRedirects` + prisma impl + MemoryDataAccess. Contracts `publicRedirect(List)Schema` additive.
- **Testler.** api-gateway **+10** (slug-governance write/duplicate/tenant/chain-collapse/second-rename/back-rename/no-op 9 + public redirect endpoint 1 → 134) · storefront **+13** (redirect-runtime 10 + middleware 3) · utils **+4** (enum-map 2 + seo-paths 2). Ayrıca TD-056 + iki latent test fixture düzeltildi → **storefront tsc TAM temiz (exit 0)**. **Tam unit suite 2097/2097 PASS** (169 dosya). **`next build` temiz** (`ƒ Middleware 34.8 kB` kayıtlı; 17 route korunur). api-gateway tsc temiz. prisma generate OK.
- **Durum.** Implementation + gate + `next build` TAMAM. **Commit/PR/merge/deploy YAPILMADI** (brief kuralı). Docker smoke (gerçek DB: PATCH slug→SlugHistory/Redirect satırı + middleware 301) = deploy adımı (TD-063). Kod worktree'de hazır.

## Faz 2C-8E — Enterprise Search Autocomplete & Discovery (TODO-156E) — worktree, ship bekliyor

**Kapsam.** Aramayı UX olarak enterprise seviyeye çıkarır (autocomplete + discovery). **Ranking/analytics/AI DEĞİL** (ADR-084 · Çalışma Sınırı). Mevcut search read-model / search API / SSR / SEO DEĞİŞMEDİ (geriye dönük uyumlu; header düz-form → combobox, native GET korunur). Migration YOK.

- **(1) Backend — AYRI suggest yolu.** `SearchProvider.suggest(storeId, SuggestQuery): SuggestResult` portu (types.ts) — tam `search`'ü çağırmaz (facet/pagination/count YOK). Postgres impl `suggest-query.ts::suggestReadModel`: bounded (products relevance-sıralı exact→prefix→hasStock→trigram; brands DISTINCT+COUNT prefix; categories prefix + kök→yaprak breadcrumb yukarı-recursive-CTE `buildCategoryPaths` SAF döngü-guard'lı; query-suggestions `buildQuerySuggestions` SAF deterministik-tekil). `postgres-provider.suggest` + index export. YALNIZ read-model (`ProductSearchDocument`+`ProductCategory`); Product/EAV re-join YOK. Kampanya rozeti read-time bastırma (PDP↔PLP↔AC tek formül).
- **(2) Gateway.** `GET /public/stores/:storeSlug/autocomplete?q=&limit=` (`autocomplete-routes.ts` + SAF `autocomplete-parser.ts`; grup limitleri bounded sabit) + process-yerel TTL cache (`autocomplete-cache.ts`, 30s, bounded LRU; `x-autocomplete-cache: hit|miss`; Redis YOK). Yanıt `publicAutocompleteResponseSchema` ALLOWLIST (storageKey/mediaId/searchText/internal SIZMAZ; kart görseli url'i tek çıkış `resolveMediaUrl`). server.ts wiring + contracts + api-client re-export.
- **(3) Storefront — proxy + combobox.** Tarayıcı gateway'e DOĞRUDAN gitmez: client → `/api/autocomplete` Next route handler → `lib/server/autocomplete.ts` BFF → gateway (gateway URL sunucu-yalnız). SAF `lib/autocomplete/*`: `cache` (client TTL+LRU), `highlight` (XSS-güvenli uzunluk-koruyucu Türkçe fold), `flatten` (`buildPopupOptions` tüm modlar tek klavye-listesi + `nextActiveIndex` wrap), `recent` (localStorage tekil/bounded), `href` (url-state kanonik). `use-autocomplete` hook: **debounce 180ms + AbortController + RACE guard + client cache**. `SearchCombobox` (ARIA combobox/listbox; ↑↓/Enter/ESC/Home/End/Tab; hover↔aktif; aria-live; no-JS `<form action=/products>` fallback; empty/results/zero modları) + `MobileSearch` tam-ekran drawer (autofocus/scroll-lock/ESC/safe-area). Header `HeaderSearch`→combobox; `site-header` + `MobileSearch`. i18n `autocomplete.*` (tr+en).
- **(4) Testler.** search-service **+9** (buildQuerySuggestions + buildCategoryPaths) → **87/87**. api-gateway **+14** (parser 7 + cache 3 + route entegrasyon 4: 404/400/allowlist/cache-hit) → **1081/1081**. storefront **+23** (highlight 6 + flatten 9 + client cache 3 + recent 5) + header-search combobox iskeleti güncellendi → **386/386**. tsc temiz (search-service/api-gateway/storefront). **`next build` 18 route** (`ƒ /api/autocomplete` kayıtlı; middleware korunur).
- **(5) Docker smoke (gerçek PG + gerçek tarayıcı).** api-gateway + storefront-web worktree'den rebuild (main stack üzerine, 5/5 healthy). API: q=demo → suggestions+products (marka "Commerce OS", fiyat, kampanya rozeti, image url türetildi) · brand grubu (Commerce OS ×2, marka-önce sıralama) · kategori breadcrumb (Accessories/Apparel) · prefix (hoo→Demo Hoodie) · **0 storageKey sızıntısı** · hata (no-q 400 / ghost store 404 / >100 400) · **cache miss→hit**. Storefront proxy `/api/autocomplete` (gerçek veri, no-q 400, Cache-Control). **Tarayıcı:** desktop dropdown (ÖNERİLER/ÜRÜNLER gruplu + highlight "Demo" bold + fiyat + görsel + "tüm sonuçlar"); **klavye** ArrowDown → `aria-activedescendant`=opt-0 + `aria-selected=true` (tam 1), ürün satırına inip **Enter → `/products/demo-hoodie` PDP** (h1 "Demo Hoodie"); **mobil** drawer (tam-ekran, "tote" → ÖNERİLER/ÜRÜNLER highlight + ₺399,00). ARIA (listbox/label/expanded/controls) doğrulandı.
- **Durum.** Implementation + gate + `next build` + Docker/tarayıcı smoke TAMAM. **Commit/PR/merge/deploy YAPILMADI** (brief kuralı; kod worktree'de hazır). Bkz. ADR-084 + TD-066…071.

## Faz 2C-8E UX Rafinasyonu (TODO-156E · 2. geçiş) — worktree, ship bekliyor

**Kapsam.** UX review sonrası autocomplete DENEYİMİ enterprise seviyeye çekildi. **Search mimarisi/API/ranking/SEO'ya DOKUNULMADI** (yalnız autocomplete UX + kendi projeksiyonu). Migration YOK.

- **(1) Fiyat kaldırıldı.** Autocomplete satın-alma ekranı değil → `SuggestProduct` + `publicAutocompleteProductSchema`'dan `minPriceMinor/maxPriceMinor/currency` ÇIKARILDI; kampanya alanı `PublicCampaignBadge` (indirim tutarı) → `hasCampaign` + `campaignLabel` (yalnız rozet; TUTAR taşımaz). suggest-query FİYAT SELECT etmez.
- **(2) Ürün kartı zenginleştirme.** Hiyerarşi ad→marka→kategori. `categoryLabel` gateway route'ta `resolveCategoryNames` (loadPublicCategoryNames) ile çözülür (search ucuyla AYNI bounded display-only desen; kategori id DTO'ya SIZMAZ). Rozetler: **Yeni** (`isNew` = productCreatedAt son 30 gün, suggest'in tek deterministik `now`'u) + **Kampanya** (nötr pill, admin `campaignLabel` yoksa i18n jenerik). Çok Satan = TD-072 (satış analitiği yok).
- **(3) Grup sırası + gruplama.** `buildPopupOptions` results sırası Öneriler → Kategoriler → Markalar → Ürünler. Boş grup başlığı render EDİLMEZ (panel grup-değişiminde başlık ekler; yalnız veri olan grup option üretir).
- **(4) "Tüm sonuçları görüntüle (N)".** `SuggestResult.total` (eşleşen ürün COUNT'u, tek ekstra sorgu) → combobox "«q» için N sonucu görüntüle"; aria-live "N ürün bulundu".
- **(5) Empty state + aktif satır.** Empty'ye "Tüm ürünlere göz at" aksiyonu (çıkmaz önlenir); popüler kategoriler = TD-073. Aktif satır yüzey + sol ink çubuğu (hover/klavye/touch birebir; tek-accent korunur). Touch hedefi min-h-11.
- **(6) i18n + rozet a11y.** tr+en `newBadge/campaignGeneric/viewAllResultsCount/resultsAnnounce` + copy iyileştirmesi. Rozetler nötr text pill (SR okur; ikonlar aria-hidden).
- **Testler.** search-service **87/87** (suggest yardımcıları aynı) · api-gateway **1081/1081** (route testi yeni şema: fiyat-yok/categoryLabel/hasCampaign/isNew/total + listCategories fake) · storefront **392/392** (+6 yeni `autocomplete-panel.test.tsx`: fiyat-yok/hiyerarşi/Yeni+Kampanya/stok/listbox + flatten sırası güncellendi). tsc temiz · `next build` 18 route.
- **(7) Docker + tarayıcı smoke.** api-gateway+storefront worktree'den rebuild. API: `keys` fiyat İÇERMEZ, `categoryLabel`=Apparel, `hasCampaign`=true/`campaignLabel`=null/`isNew`=true, `total`=2, 0 storageKey/price sızıntısı. Tarayıcı (desktop): dropdown ÖNERİLER+ÜRÜNLER, ürün satırı **[YENİ][KAMPANYA]** nötr pill + "Commerce OS · Apparel" + **FİYAT YOK**, footer "«demo» için 2 sonucu görüntüle"; "comm" → grup sırası **Öneriler → Markalar → Ürünler** (Kategoriler boş→gizli) + marka "2 ürün" count. HEPSİ PASS.
- **Durum.** Implementation + gate + `next build` + Docker/tarayıcı smoke TAMAM. **Commit/PR/merge/deploy YAPILMADI** (brief kuralı). Bkz. ADR-084 (UX Rafinasyonu) + TD-072/073.
