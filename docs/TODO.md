...
- **TODO-176B E2E Regression Coverage — PR2 (BUG-CART-006 fix + regression suite) — IN REVIEW** (2026-08-09). **BUG-CART-006 (reorder → boş sepet) DÜZELTİLDİ + kalıcı @smoke testine bağlandı (RED→GREEN kanıtlı).** **Root cause:** `buyAgainAction` (`apps/storefront-web/lib/server/order-actions.ts`) hibrit cart'a (TODO-167/ADR-266) göre güncellenmemişti — oturum açmış müşteride bile sepete KOŞULSUZ cookie'ye (`writeCartItems`) yazıyordu; oysa auth cart DB-otoriter (`getAuthCartProjection` → nav rozeti + `/cart` + checkout hepsi DB cart okur). Reorder DAİMA authenticated (order detail `x-customer-session` ister) → cookie yazımı yanlış defter → UI "eklendi" der ama rozet artmaz + `/cart` boş. **Fix:** auth oturumda satırlar `authAddLine` ile KALICI DB cart'a yazılır (persist edilemezse "mevcut değil" sayılır; sessiz fallback YOK); anonim yol savunma amaçlı cookie'de korunur. Invariant `reorder == /cart == checkout` `tests/e2e/smoke/08-reorder-cart-invariant.spec.ts` ile kanıtlandı. **Yeni `regression` Playwright projesi** (`@regression`, `pnpm e2e:regression`, smoke gate DIŞINDA): (1) Alışveriş Bakiyem finansal invariant (gösterilen available == goodwill grant; STORE_CREDIT nakde çevrilmez) `regression/01-shopping-balance.spec.ts`; (2) refund destination ORIGINAL_PAYMENT/SHOPPING_BALANCE + İadelerim immutable snapshot `regression/02-returns-refund-destination.spec.ts`. **Fixture (additive, idempotent, deterministik):** `e2e-order-2001` (reorder: tshirt M × 2), goodwill kredi grant (account+lot+ledger), 2 `ReturnRequest` (farklı hedef/statü). **Gate:** typecheck + lint (0 hata) + workspace test Run1+Run2 (42/42; api-gateway 2488) + build 27/27 + `git diff --check` temiz + Playwright smoke 9/9 + regression 3/3 (worktree :3100 + docker gateway :4000 + `e2e-store`). **Kalan (TD-176-1):** shopping-balance-only/mixed ödeme, cancellation, return submit, wishlist, ürün review, order-experience review (her biri non-idempotent mutasyon/çok-aktör → dedike fixture+teardown; ayrı PR). Bkz. `docs/TECHNICAL_DEBT.md` TD-176-1.
- **TODO-176 Automated Storefront E2E Regression Suite — PR1 CLOSED & DEPLOYED** (2026-08-09; PR #205 merge `9a1eca9`; ADR-287; CI `smoke` green (4m2s, full docker stack + seed + 8 smoke) + `lint·test·build` green; post-deploy prod-smoke green vs enterprise-demo :3000 — home `<title>` fix canlı doğrulandı). Manuel browser smoke'un release gate'in **ana** mekanizması olmaktan çıkarılıp repo-içi kalıcı Playwright E2E paketi kurulması. **Konum:** kök `tests/e2e/` + kökte `playwright.config.ts`. **Projeler:** `setup` (gerçek UI login → `tests/e2e/.auth/customer.json` storageState), `smoke` (Desktop Chromium, `@smoke` grep, storageState reuse, `setup`'a bağımlı — 8 çekirdek senaryo), `responsive` (`@responsive`, viewport **375/1440**, küçük kritik subset, `setup`'a bağımlı), `prod-smoke` (`@prod-smoke`, **anonim** — storageState/setup YOK — post-deploy hedef ortama karşı güvenli/read-only). `workers:1` zorunlu: tüm smoke/responsive testleri aynı e2e müşterisinin DB-tabanlı (server-authoritative) sepetini paylaşır (`fixtures/cart.ts`), `fullyParallel:false` yalnız dosya-içi sırayı garanti eder — dosyalar-arası izolasyon YOK. **Store runtime (kritik):** storefront tek-mağaza pinned (`STOREFRONT_DEMO_STORE_SLUG`); docker `storefront-web` servisi kaynağı volume MOUNT ETMEZ (build context=ana repo) → worktree değişiklikleri o servise yansımaz. Bu yüzden **LOCAL** e2e storefront = host `pnpm e2e:storefront` (`next dev --port 3100`, worktree kodu, `e2e-store` slug); **CI** = `infra/docker/docker-compose.e2e.yml` `storefront-web-e2e` servisi (branch checkout'undan build, :3100). İkisi de `E2E_STOREFRONT_URL=http://localhost:3100` hedefine çalışır; enterprise-demo (:3000) DOKUNULMAZ. **Fixture:** dedike `packages/db/scripts/e2e-seed.mjs` (`db:seed-e2e`, idempotent, `APP_ENV` guard'lı) izole `e2e-store` kurar — test müşterisi `e2e-customer@example.test`, ürünler `e2e-tshirt`(3 varyant)/`e2e-mug`, kupon `E2E10`, bir `CustomerAddress`, seed sipariş `e2e-order-1001`; deterministik id'ler `tests/e2e/fixtures/ids.ts`. Write-path verisi (sepet vb.) test içinde üretilir; sepet izolasyonu `clearCart()` (`fixtures/cart.ts`) `beforeEach`'te. **8 çekirdek smoke senaryosu (PR1):** auth/session · PDP variant seçimi · add-to-cart · header cart badge · cart refresh/persistence · coupon apply/remove+repricing (claim→kullan iki adımlı) · cart→checkout canonical identity · order list/detail — her biri iş-sonucu assert eder (persistence, doğru variant/qty/fiyat/count, ham enum yok, console/runtime hatası yok). **Env:** `E2E_STOREFRONT_URL`(default :3100)/`E2E_GATEWAY_URL`(default :4000)/`E2E_STORE_ADMIN_URL`(default :3002); prod-smoke: `E2E_PROD_PRODUCT_SLUG` ZORUNLU (eksikse fail-loud config error, silent fallback YOK), `E2E_PROD_CATEGORY_SLUG`/`E2E_PROD_SEARCH_TERM` opsiyonel (eksikse görünür `test.skip`) — kategori/arama ikisi de aynı `/products?category=`/`/products?q=` PLP rotası. **Flakiness policy:** arbitrary sleep yasak (web-first `expect`), `retries:CI?2:0`, kalıcı skip yasak. **CI gate:** ayrı `.github/workflows/e2e.yml` job `smoke`, required-status-check context **`smoke`** (governance — branch protection'a eklemek repo-admin adımı, ayrı dokümante edildi), başarısız artifact (playwright-report+test-results, `.auth/` HİÇBİR ZAMAN), `if:always()` cleanup. **npm scripts:** `e2e:install`/`e2e:smoke`/`e2e:responsive`/`e2e:prod-smoke`/`e2e:report`/`e2e:storefront`/`db:seed-e2e`/`db:cleanup-e2e`. **Docs:** [ADR-287](adr/ADR-287-playwright-e2e-release-gate.md), `docs/TESTING.md` (kullanım kılavuzu + Definition of Done), `tests/e2e/README.md` (quickstart). **PR2 (FUTURE, bu PR'a dahil DEĞİL):** reorder/BUG-CART-006 invariant, shopping-balance-only ödeme, mixed balance+external ödeme, cancellation, return, refund ORIGINAL_PAYMENT, refund SHOPPING_BALANCE, İadelerim, Alışveriş Bakiyem, wishlist, ürün review, order-experience review — bkz. `docs/TECHNICAL_DEBT.md`. **Governance (TD-176-5) — RESOLVED (2026-08-09):** main branch protection etkinleştirildi (`required_status_checks.contexts=["smoke","lint · test · build"]`, `strict:true`) — artık `smoke` (E2E gate) yeşil olmadan main'e merge **bloklanır**.

- **TODO-174B.2 Order Experience Recovery — Case Detail & Operations — ✅ CLOSED & DEPLOYED** (PR #204 merge `79757b9`; 2026-08-09). CI (lint·test·build) 6m17s PASS. Deploy: yeni migration YOK (şema DEĞİŞMEDİ); docker `api-gateway`+`store-admin-web` main'den (`79757b9`) rebuild+recreate (`--no-deps --force-recreate`; postgres/redis/worker/storefront-web/admin-web DOKUNULMADI) → 2 servis healthy. Post-deploy smoke (deployed :4000/:3002): api-gateway `/health` 200 · yeni uç `order-experience/assignable-users` → 401 (wired+admin-gated, 404 DEĞİL) · order-experience list → 401 · store-admin `/order-experience` → 307 (auth redirect) · `edm-store` case'siz 1-2★ orphan=0 (backfill kalıcı). **Root cause (kanıtlı, git):** `createOrderExperienceReview` (müşteri review-submit, `6dde962`/TODO-174A) auto-case açan `openRecoveryCaseForReview`'den (`e15b809`/TODO-174B F4) ÖNCE deploy edildi → iki deploy arasında girmiş 1-2★ review'lar case'siz kaldı (Sipariş Deneyimi listesinde Recovery/SLA/İşlem=—). Audit: liste+`[caseId]` detay+lifecycle+goodwill ZATEN merged (PR #194/#195); gerçek iş = orphan-veri onarımı + kalite çıtası + ASSIGN bug (SIFIRDAN inşa DEĞİL). **(1) Backfill:** `openRecoveryCaseForReview` `openedAt` override + `backfillMissingRecoveryCasesForStore` (idempotent; `@@unique`+P2002; SLA gerçek `review.createdAt`'ten hesaplanır) + tsx CLI (`apps/api-gateway/scripts/backfill-recovery-cases.ts`; dry-run default, `--apply` için `--store` ZORUNLU). Canlı onarım: `edm-store scanned=1 created=1`, tekrar `scanned=0` (idempotent); 3★/4-5★ kapsam DIŞI. **(2) ASSIGN bug:** frontend literal `"me"` DB'ye yazılıyordu → backend `assigneePlatformUserId==="me"` → gerçek `actorPlatformUserId`; read-model assignee join (PlatformUser adı; ham id/"me" UI'a sızmaz). **(3) Detail projection:** `customer.phone` + order ödeme/iade özeti (totalMinor/currency/paymentStatus/shoppingCreditUsedMinor) + case goodwill toplamı (RECOVERY_GOODWILL). **(4) Assignable users:** `GET /stores/:storeId/order-experience/assignable-users` (StoreUser↔PlatformUser, storeId-scoped, cross-store izole; "Kullanıcıya ata" dropdown besler). **(5) Paylaşılan `recovery-labels` modülü (store-admin):** status/priority/activityType/outcome/resolution/cancelReason(gerçek `OrderCancellationReason` 18 değer)/orderStatus/paymentStatus + `slaState` (İçinde/Bugün bitiyor/Gecikti/Tamamlandı) — ham enum UI'a ASLA çıkmaz (bilinmeyen değer defansif humanize). **(6) Detail page:** müşteri/sipariş detay linkleri + telefon + ödeme/iade özeti + timestamps (opened/firstContact/due/resolved/closed) + SLA rozeti + OTHER→note client guard + bağımsız "Not ekle" (NOTE) + "Kullanıcıya ata" dropdown + "Kendime ata". **(7) List page:** assignee kolonu + SLA durum ayrımı. **TDD:** 15 order-recovery integration (backfill idempotent + ASSIGN-me + assignee-join list/detail + phone/finansal/goodwill + assignable cross-store) + 12 recovery-labels unit. **Gate:** db:generate + typecheck (turbo build + tsc --noEmit temiz) + lint 0 hata + workspace test Run1+Run2 (api-gateway 2659 · store-admin-web 380) + `git diff --check` temiz. **Browser smoke** (worktree api-gateway :4001 + store-admin-web :3100, gerçek-auth cookie-inject izole fixture `smoke-tdfr7`, 375/768/1024/1440): liste Recovery("Açık")+Atanan kolonu+SLA+Detay butonu · detay TAM lifecycle Kullanıcıya-ata(isim "Demo Platform Admin")→Arandı(firstContact)→Sorun dinlendi(outcome "Teslimat şikâyeti"; OTHER+boşnot Kaydet disabled DOĞRULANDI)→bağımsız Not→Alışveriş bakiyesi ₺150(detail "Tanımlanan bakiye"de görünür)→Çözüldü→Kapat(aksiyonlar gizli) · 7-kayıt append-only timeline · listeye dön status=CLOSED/Atanan/Tamamlandı · ham enum YOK · 4 viewport yatay taşma YOK · recovery API istekleri hepsi 200 (console 4×401 login-öncesi auth-probe, recovery ile ilgisiz) · fixture cascade temizlendi (enterprise-demo PRISTINE). Şema DEĞİŞMEDİ → migration YOK; docker DB=worktree DB olduğundan backfill deploy'dan önce zaten canlı uygulandı.
- **TODO-175 Refund Destination Choice — ✅ CLOSED & DEPLOYED** (PR #203 merge `fa30575`; ADR-285/286; 2026-08-08). CI (lint·test·build) 5m54s PASS. Deploy: migration `20260808110000_todo175...` prod DB'de canlı ("up to date"; finished_at dolu); docker `api-gateway`+`storefront-web`+`store-admin-web` main'den (`fa30575`) rebuild+recreate (`--no-deps --force-recreate`; postgres/redis/**worker DOKUNULMADI** — uptime'lar korundu 9/9/6 gün) → 3 servis healthy. Post-deploy smoke (deployed :4000/:3000/:3002): api-gateway `/health` 200 · cancel-eligibility (unauth) 401 (wired+session-gated, 404 DEĞİL) · storefront 200 · store-admin 307. **Browser smoke (worktree :4100/:3100/:3200, gerçek-auth izole fixture, 375/768/1024/1440):** mixed order cancel modal 3-adım stepper (İPTAL NEDENİ→İADE YÖNTEMİ→ÖZET) · ORIGINAL_PAYMENT confirm split (Alışveriş bakiyesi ₺300 + Ödediğim yönteme iade ₺700) · SHOPPING_BALANCE confirm (Alışveriş bakiyenize eklenecek ₺1.000) · execution SUCCEEDED → Alışveriş Bakiyem ₺1.000 (refund-origin hareketler, human copy) · İadelerim "İade hedefi: Alışveriş bakiyesine" · return wizard REFUND→İADE YÖNTEMİ alt-seçimi · store-admin İadeler "İADE YÖNTEMİ" kolonu=Alışveriş bakiyesi + order detay "İade yöntemi + dahili kredi" notu · 4 viewport yatay taşma YOK · raw enum YOK · storefront console error YOK · fixture FK-güvenli temizlendi (enterprise-demo PRISTINE). Smoke sırasında bulunan minor copy defect'i (refund-to-balance hareketinde OS-{n} boş) fix'lendi (`issueCreditInTx` orderId attribution; PR'a dahil). Müşteri iade/iptalinde geri ödemenin **orijinal ödeme yöntemine** mi yoksa **alışveriş bakiyesine** mi gideceğini seçebilir. **Ürün invariant'ları:** `STORE_CREDIT value cannot be converted to cash/original PSP refund.` + `Customer may voluntarily convert eligible externally-paid refund into Shopping Balance.` **Çekirdek model:** her refund `R`, kalan refundable havuzlara **oransal** olarak `Rc` (credit-origin → HER ZAMAN shopping balance restore) + `Re` (external-origin → müşteri seçer) bileşenlerine ayrılır (`refunds/destination-calc.ts`; BigInt ara matematik + safe-integer guard). **Şema (additive migration `20260808110000_todo175...`):** `RefundDestination` enum; `ReturnResolutionType += REFUND` (nötr; legacy `REFUND_TO_ORIGINAL_PAYMENT` korunur, `isRefundResolution`/`resolveEffectiveRefundDestination` ile map); `CustomerCreditLot.expiresAt` **nullable** (null=non-expiring refund-origin credit); `ReturnRequest.{refundDestination,SelectedBy=CUSTOMER,SelectedAt}` immutable snapshot; `OrderRefund.refundDestination`; `RefundExecutionMode += INTERNAL_CREDIT`; `CreditSourceType += ORDER_RETURN`; `CreditLedgerType += RETURN_CREDIT_RESTORE`. **Execution:** SHOPPING_BALANCE external legi = PSP çağrısı YOK → `OrderRefund` INTERNAL_CREDIT tx-içi SUCCEEDED + `issueCreditInTx(expiresAt=null, REFUND_RESTORE)`; ORIGINAL_PAYMENT = mevcut PSP akışı (Re tutarında); credit-origin `Rc` cancellation→`restoreCreditForOrderInTx` (expired no-revive korunur), return→`restoreCreditAmountForOrderInTx` (**expired original lot → cash'e çevirmeden yeni non-expiring reissue**, ADR-286). **İki-ledger completion:** `Σ SUCCEEDED OrderRefund + Σ return credit restore ≥ intent.total`. **Düzeltmeler:** (A) geçersiz hedef SESSIZ fallback YOK → `INVALID_DESTINATION`/`REFUND_DESTINATION_INVALID`; (B) `expiryDays=null` yalnız allowlisted refund-origin sistem yolu; (C) safe-integer BigInt money; (D) adoption rate paydası yalnız choice-eligible refund. **UI:** storefront cancel modal (koşullu "İade yöntemi" adımı + split confirm) + return wizard (REFUND + destination alt-seçimi) + İadelerim/Alışveriş Bakiyem (refund-origin copy, raw enum yok); store-admin return/cancellation detail (destination + INTERNAL_CREDIT allocation) + unified İadeler kolonu. **TDD:** 109 yeni/genişletilmiş TODO-175 test (pure allocation/expiry/provenance + integration cancellation×5/return×8/credit-restore×4/create×3/visibility×adoption; gerçek-DB, store/customer izolasyon). **Gate:** db:generate + fresh migration replay (throwaway DB) + existing DB upgrade + full workspace typecheck + lint 0 hata + targeted 109 + **workspace test Run1+Run2 (42/42 task; api-gateway 2652)** + build 27/27 + `git diff --check` temiz. **Real-env doğrulama:** worktree :4100/:3100 gerçek-auth izole fixture — mixed order cancel-eligibility gateway'den doğru split döndürdü (`externalRefundableMinor:70000` + `creditRestorableMinor:30000` + `offerOriginalPayment/Balance:true`) + `/me` auth; fixture FK-güvenli temizlendi (enterprise-demo PRISTINE). **Açık:** merge + prod deploy + tam 4-viewport UI browser click-through (ad-hoc kurulumda STORE_SLUG çözümleme quirk'i nedeniyle standart smoke harness'ında tamamlanmalı). Bkz. [ADR-285](adr/ADR-285-refund-destination-choice.md), [ADR-286](adr/ADR-286-refund-origin-non-expiring-credit.md), `docs/superpowers/specs/2026-08-08-todo-175-refund-destination-choice-design.md`.
