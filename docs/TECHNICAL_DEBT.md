...
- **TD-051.1 — Dynamic Facet UI YOK (→ TODO-156C).** Backend disjunctive facet (`selectionMode`/`dataType`/`colorHex`/range) döndürür; bu fazda facet **render edilmez** (sidebar/drawer/çip yok). Ancak URL'de `filter[...]` varsa codec **eksiksiz passthrough** eder ve endpoint doğru sonuç döner (test edildi). 156C: `FacetRenderer` registry + `FilterRail`/`FilterDrawer` + `ActiveFilterChips`.
...
- **TD-051.3 — Kampanya rozeti YOK (→ TODO-155.2, TD-050.1).** Search read-model kampanya rozeti (F4A "Sepette %X") taşımaz; kart compareAt indirimi + discountPercent + Omnibus'u gösterir. Rozet 155.2'de read-model'e eklenince kart otomatik tüketir (adapter'da kanca hazır — `discountPercent` deseni).
...
- **Durum.** DONE (worktree; commit/PR/deploy YOK — brief gereği). Gate: storefront 273/273 (+75) · i18n 47 (TR/EN parity) · contracts 110 · next build yeşil · lint temiz · Docker runtime smoke ALL PASS. YENİ MIGRATION YOK. Bloklayıcı: HAYIR.
