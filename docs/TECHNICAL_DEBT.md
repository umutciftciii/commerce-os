...
- **TD-091 — `GET /stores/:id/inventory/matrix` SINIRSIZ; Envanter ekranı hâlâ tamamen istemci-taraflı.** Uç, mağazadaki tüm non-archived varyantları + bakiyelerini tek yanıtta döner (enterprise-demo: 2202 varyant) ve `/inventory` ekranı arama/durum filtresini istemcide uygular. Ortak Data Grid'e taşımak yalnız UI değişikliği DEĞİLDİR: `inventoryStoreMatrixResponseSchema` sayfalama meta'sı taşımıyor, `listStoreVariants` sayfalanabilir değil ve stok "izleme merkezi" semantiği (TODO-152A) tüm satırların aynı anda görünmesine dayanıyor. Doğru çözüm: matris ucuna `page/pageSize/search/warehouse` + ürün-bazlı gruplama sözleşmesi eklemek. Bugünkü risk: büyük katalogda ekran ağır yüklenir (sayfa donmaz ama ilk boyama gecikir). **Öncelik: YÜKSEK** — kataloğu büyüyen ilk müşteride hissedilir.
...
- **TD-093 — Ürün/kategori SEÇİCİLERİ hâlâ ilk 100 kayıtla sınırlı.** Kampanya ürün seçici ve home showcase seçici `listProducts()`'ı, kategori ebeveyn seçici ve ürün filtresi `listCategories({pageSize:100})`'ü kullanır. Sayfalama artık görünür olduğu için "sessiz kesme" defekti liste ekranlarında kapandı, ancak SEÇİCİLERDE sürüyor: 100'den fazla ürünü olan mağazada kampanyaya eklenecek ürün seçicide bulunamayabilir. Doğru çözüm sayfalama değil, sunucu-taraflı **arama tabanlı seçici** bileşenidir (autocomplete deseni; ADR-084 combobox'ı yeniden kullanılabilir). Bu iş paketinde bilinçli olarak yapılmadı: seçici bileşeni ayrı bir UX yüzeyidir ve PR'ı kontrolsüz büyütürdü. **Öncelik: YÜKSEK.**
...
- **TD-095 — Medya kütüphanesi sabit 100 kayıtla sınırlı ve pagination meta'sı YANILTICI.** `GET /stores/:id/media` `take: 100` sabitiyle çalışır, `offset` hiç uygulanmaz ama yanıt `{limit:100, offset:0, total}` döndürür — yani meta "sayfalama var" izlenimi verir. 100'den fazla görseli olan mağazada eski görseller MediaUpload kütüphanesinden seçilemez (yalnızca yeniden yükleme ile eklenebilir). Çözüm: uca ortak `page/pageSize` + (opsiyonel) arama sözleşmesini uygulamak ve modal'a sayfalama çubuğunu bağlamak. **Öncelik: ORTA-YÜKSEK** (görsel yönetimi Faz 2/3 ile canlıya girdi).
