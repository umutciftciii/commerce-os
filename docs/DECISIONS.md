...
- Karar: admin-web'in kanitlanmis BFF deseni store-admin-web'e tasinir (ortak paket yerine app-yerel
  ...
  boylece tarayici keyfi mağaza secemez. (Faz E1 ile bu geçici model kaldırıldı: aktif mağaza artık
  kimlik doğrulanan StoreUser oturumundan — `session.storeId`, gateway `/auth/store/session` — türetilir;
  BFF hiçbir slug env okumaz ve `admin.stores.list`/"ilk mağaza" fallback'i yoktur.) (3) Tum gateway cagrilari ayni-origin `/api/*` proxy
...
## ADR-092 — Inventory Matrix Scalability: sunucu-otoriter mağaza-geneli stok liste sözleşmesi (page/pageSize/search/sortBy/sortOrder + filtreler), tek raw SQL CTE tarama, sayfadan bağımsız summary; SAF hesap otoritesi korunur (TODO-159C)

**Tarih:** 2026-07-23 · **Durum:** Kabul edildi (uygulandı) · **Öncül:** ADR-089 (Admin Data Grid),
ADR-090 (Admin Searchable Selector), ADR-076 (Inventory Engine + çift otorite), TODO-152A (stok izleme
merkezi) · **Kapattığı borç:** TD-091 · **Açtığı sınırlar:** TD-099, TD-100

### Bağlam

TODO-152A mağaza-geneli SALT-OKUMA stok matrisini (`GET /stores/:id/inventory/matrix`) kurmuştu, ama
uç mağazadaki TÜM non-archived varyantları + bakiyelerini TEK yanıtta döndürüyordu (enterprise-demo:
2.202 varyant, ~800 KB–1.5 MB) ve `/inventory` ekranı arama/durum filtresini İSTEMCİDE, KPI'ları da tüm
dataset üzerinden uyguluyordu (TD-091). Bu yalnız bir UI sorunu değildi: `inventoryStoreMatrixResponseSchema`
sayfalama meta'sı taşımıyordu, `listStoreVariants` sayfalanabilir değildi ve izleme merkezi semantiği tüm
satırların aynı anda görünmesine dayanıyordu — üçü birlikte ele alınmalıydı.

### Karar

1. **Ekran ADR-089 Data Grid standardına taşındı** (yeni paralel tablo altyapısı YOK). Query:
   `page`/`pageSize`/`search`/`sortBy`/`sortOrder` + `warehouseId`/`stockStatus`/`reserved`/`variantStatus`/
   `productStatus`. Response: `warehouse` + bir SAYFA `rows` + ortak `adminListPaginationSchema` meta'sı +
   sayfadan BAĞIMSIZ `summary`. `sortBy` KAPALI allowlist'tir (serbest metin ASLA `orderBy`'a geçmez);
   `pageSize` tavanı (100) SUNUCUDA zorlanır; geçersiz sort/filtre/pageSize 400 verir.

2. **Tek raw SQL CTE tarama** (`buildStoreMatrixScan`, `apps/api-gateway/src/inventory-engine/data.ts`).
   `base` CTE resolved skaler kolonları (ProductVariant + Product join + InventoryBalance LEFT JOIN
   [unique index `warehouseId,variantId`] + DEFAULT depoda InventoryItem LEFT JOIN); `calc` CTE
   türetilmiş `rawAvailable`/`sellable`/`stockStatus`'ü üretir. Sabit 3 sorgu: (1) sayfa taraması
   (LIMIT/OFFSET), (2) summary aggregate (COUNT + SUM + FILTER; count dahil), (3) sayfa-id'leri için
   attribute hidrasyonu (tek `findMany`). **N+1 YOK, sınırsız tarama YOK.** `buildAdminProductScanSql`
   deseniyle aynı otorite: `ILIKE … ESCAPE '\'` wildcard kaçışı, `NULLS LAST`, ikincil `productId`/
   `variantId` anahtarı (deterministik sayfa sınırı, aynı ürünün varyantları bitişik).

3. **Çift otorite (ADR-076) SQL'de birebir korunur.** DEFAULT depoda `onHand`/`reserved` CANLI
   `InventoryItem`'dan overlay edilir (`COALESCE(ii.…, ib.…, 0)` — item satırı varsa onun değeri, yoksa
   balance, o da yoksa 0); non-default depoda yalnız `InventoryBalance`. `incoming`/`safetyStock`/
   `reorderPoint` her zaman balance'tan. Checkout/storefront ile sıfır regresyon.

4. **Stok formülü tek GÖSTERİM otoritesi SAF motordur.** Satır `currentCalc` serviste yine
   `computeCalc` (`availability.ts` + `calculator.ts`) ile hesaplanır. SQL'deki `available`/durum
   türetmesi YALNIZ filtre/sıralama/summary içindir ve calculator'ın birebir transkripsiyonudur
   (parite: contract + calculator birim testleri + canlı summary tutarlılığı; formül İKİ dilde
   yaşadığı için TD-100 olarak kaydedildi). `available` materialize EDİLMEZ (türetilir; ADR-076).

5. **Summary sayfadan BAĞIMSIZDIR:** aktif filtreye uyan TÜM kümenin aggregate'i (ayrı sorgu).
   `totalVariants` = `pagination.totalItems`. KPI'lar bu summary'den gelir (25 satırın toplamı DEĞİL).
   Dashboard özeti de artık matris `summary`'sinden türetilir (`pageSize=1` ister; eski "ilk sayfa"
   tech-debt'i kapandı).

6. **Additive indeks:** `ProductVariant(storeId, status)` (migration `20260723120000`). Matrisin
   birincil filtresini (`storeId` + `status<>ARCHIVED`) tek erişimle karşılar; çok-tenant ölçekte
   (50k+ varyant, tek store küçük bir kesir) taranan kümeyi daraltır. Enterprise-demo'da tek store
   tablonun ~%99'u olduğundan planner haklı olarak seq scan seçer (indeks zararsız; ölçekte devreye
   girer). Sıralama türetilmiş kolonlarda olduğundan indekslenmez — sayfa boyutlu sonuçta top-N sort
   kabul edilebilir (5.7 ms).

7. **Bulk action semantiği (kapsam ayrımı):** Global ekranda gerçek bulk yazma YOKTUR (ADR-076: yazma
   ürün-scoped kalır). Yalnız güvenli tek-satır hızlı işlem (+/−/sıfırla) ürün-bazlı preview→apply'ı
   çağırır. Sahte bulk EKLENMEDİ; ancak Data Grid seçim altyapısı (opsiyonel `selectedIds`/
   `onSelectionChange`) sonradan GERÇEK bulk için hazırdır. İleride bulk eklenirse kapsam AÇIKÇA
   ayrılmalı: "görünen sayfa" (yalnız seçilen satırlar) vs "filtreye uyan tüm kayıtlar" (ayrı, açık
   bir kullanıcı aksiyonu). Sessiz kısmi/tam uygulama YASAK.

### Geriye uyumluluk

Response `warehouse`/`rows` alanlarını KORUR; `pagination`/`summary` + satır `barcode`/`updatedAt`
EKLENİR (additive). `rows` semantiği "tüm satırlar" → "bir sayfa" değişti; tek gerçek tüketici
(store-admin `/inventory` + dashboard özeti) aynı PR'da güncellendi. Silent truncation OLUŞMAZ:
yanıt `pagination` meta'sı taşır (`totalItems > rows.length` her zaman görünür). api-client
`storeMatrix(storeId, token, query)` imzasına geçti (products.list ile aynı desen).

### Sonuç

Canlı doğrulama enterprise-demo (edm-store, 2.138 non-archived varyant) ile: sayfa taraması **5.7 ms**
(top-N heapsort → yalnız 25 satır), payload **819 KB → 9.7 KB (~84×)**, summary `totalVariants`=2.138
...
## ADR-287 — Playwright E2E Suite as the Source-of-Truth Release Gate (TODO-176 PR1) — ACCEPTED & DEPLOYED (2026-08-09; PR #205 merge 9a1eca9)
