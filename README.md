...
### Ürün satış modeli (Faz 2F)

Ürünler ekranı F2D satış modeli alanlarını yönetir:

- Listede kompakt **Satış** kolonu: satış tipi rozeti (Online satış / Fiyat sor / Randevu al /
  WhatsApp ile sor / Sadece katalog), altında fiyat görünürlüğü · birincil aksiyon ve satın
  alınabilirlik göstergesi (**Sepete eklenebilir** / **Sepete eklenemez**).
- Ürün oluştur/düzenle formunda **Satış davranışı** bölümü: satış tipi, fiyat görünürlüğü, birincil
  aksiyon, purchasable/inquiry/appointment/whatsapp toggle'ları, min/max sipariş adedi ve opsiyonel
  CTA etiketi, WhatsApp şablonu, fiyat sorma formu başlığı, randevu notu.
- Satış tipi değiştiğinde backend tutarlılık kurallarına uygun güvenli varsayılanlar uygulanır (örn.
  INQUIRY → satışa kapalı + "Fiyat iste"); yazdığınız metin alanları korunur. Tutarsız kombinasyonlar
  ve `PRODUCT_NOT_PURCHASABLE` gibi guard hataları UI'da TR/EN lokalize gösterilir.
