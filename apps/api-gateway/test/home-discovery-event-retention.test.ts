/**
 * TODO-162 (ADR-205) — Home Discovery event retention SERVİSİ testleri (in-memory persistence double).
 *
 * Kapsam: dry-run yazma yapmaz · apply 180-gün cutoff siler (createdAt) · yakın satır korunur · store scope ·
 * circuit breaker · batch delete sınırları · SKIPPED_LOCKED + job log (jobName/queueName ayrı domain).
 */
...
import {
  ...
  type JobLogClient,
} from "../src/home/discovery-event-retention-service.js";
import type { DiscoveryEventRetentionPersistence } from "../src/home/discovery-event-retention-persistence.js";
...
function createPersistenceDouble(initial: Row[]) {
  ...
  const persistence: DiscoveryEventRetentionPersistence = {
...
function service(
  ...
  return {
    ...double,
    svc: createDiscoveryEventRetentionService({
...
describe("home-discovery-event retention service", () => {
  const baseRows = (): Row[] => [row("store_a", 181), row("store_a", 179)];
  ...
  it("job log jobName/queueName ayrı domain", async () => {
    ...
    expect(jobLog.rows[0].jobName).toBe("home-discovery-event-retention");
    expect(jobLog.rows[0].queueName).toBe("home-discovery-events");
  });
});
