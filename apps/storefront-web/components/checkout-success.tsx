...
import { ButtonLink, Eyebrow, Heading, ProductMediaFrame, Text } from "./ui";
        ...
        <ul className="mt-3 space-y-2.5 border-t border-line pt-3 text-sm">
          {confirmation.lines.map((line, index) => (
            <li key={`${line.title}-${index}`} className="flex items-center gap-3">
              {/* TD-173 — Ortak ürün medya çerçevesi (line-thumbnail). handle=title:
                  confirmation satırında productSlug YOK (bilinçli) — placeholder deterministik. */}
              <ProductMediaFrame
